...
export default function QuizShare({ quizId }: { quizId: string }) {
  ...
  return (
    <div className="share-ui glass p-4 flex flex-col items-center gap-3">
      <button className="link-button link-button-primary" onClick={copyToClipboard}>
        {copied ? "Link copied!" : "Copy Quiz Link"}
      </button>
      <div className="flex gap-4">
        <Link href={tweetUrl} target="_blank" rel="noopener noreferrer" className="link-button link-button-secondary" aria-label="Share on Twitter">
          {/* Simple Twitter SVG */}
        ...
        <Link href={fbUrl} target="_blank" rel="noopener noreferrer" className="link-button link-button-secondary" aria-label="Share on Facebook">
          {/* Simple Facebook SVG */}
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="currentColor">
